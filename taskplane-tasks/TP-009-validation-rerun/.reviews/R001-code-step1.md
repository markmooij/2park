...
- **Credential logging:** `RESULTS.md` embeds `**Bearer Token:** your token` in plain text. This follows the existing project pattern (token is in PROMPT.md) so it is not a regression, but worth noting.
